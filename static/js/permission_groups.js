...
(function () {
    ...
    function selectGroup(id) {
        if (isDirty && activeGroupId !== id) {
            Enjaz.confirmAction('لديك تغييرات غير محفوظة. هل تريد الانتقال وفقدانها؟', 'تغييرات غير محفوظة')
    ...
    function _doSelectGroup(id) {
        ...
        fetch(API.detail(id))
            .then(r => r.json())
            .then(resp => {
                if (!resp.success) { Enjaz.toast('تعذّر تحميل البيانات', 'error'); return; }
                activeGroupData = resp.data;
                populateEditor(resp.data);
                isDirty = false;
                updateDirty();
            })
            .catch(() => Enjaz.toast('خطأ في الاتصال', 'error'));
    }
    ...
    function saveGroup() {
        ...
        if (!name) { Enjaz.toast('يرجى إدخال اسم المجموعة', 'error'); editorNameInput.focus(); return; }
        ...
        fetch(API.update(activeGroupId), {
            ...
            .then(resp => {
                ...
                if (resp.success) {
                    ...
                    Enjaz.toast(resp.message || 'تعذّر الحفظ', 'error');
                }
            })
            .catch(() => { btnSave.disabled = false; btnSave.innerHTML = origHtml; Enjaz.toast('خطأ في الاتصال', 'error'); });
    }
    ...
    function deleteGroup() {
        ...
        Enjaz.confirmAction(`هل تريد حذف المجموعة "${name}"؟ لا يمكن التراجع.`, 'حذف المجموعة')
            .then(confirmed => {
                if (!confirmed) return;
                fetch(API.delete(activeGroupId), {
                    ...
                    .then(resp => {
                        if (resp.success) {
                            ...
                            Enjaz.toast(resp.message || 'تعذّر الحذف', 'error');
                        }
                    })
                    .catch(() => Enjaz.toast('خطأ في الاتصال', 'error'));
            });
    ...
    createGroupForm.addEventListener('submit', e => {
        ...
        fetch(API.create, {
            ...
            .then(resp => {
                ...
                if (resp.success) {
                    Enjaz.toast(resp.message || 'تم الإنشاء', 'success');
            ...
            .catch(() => {
                createGroupSubmit.disabled  = false;
                createGroupSubmit.innerHTML = origHtml;
                Enjaz.toast('خطأ في الاتصال', 'error');
            });
    });
    ...
    btnDiscard.addEventListener('click', () => {
        ...
        Enjaz.confirmAction('هل تريد تجاهل التغييرات والعودة للبيانات الأصلية؟', 'تجاهل التغييرات')
            .then(confirmed => {
                if (!confirmed) return;
